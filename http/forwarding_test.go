...
func testHTTP_Forwarding_Stress_Common(t *testing.T, parallel bool, num uint32) {
	...
	totalOps := new(atomic.Uint32)
	successfulOps := new(atomic.Uint32)
	key1ver := new(atomic.Int32)
	key1ver.Store(1)
	key2ver := new(atomic.Int32)
	key2ver.Store(1)
	key3ver := new(atomic.Int32)
	key3ver.Store(1)
	numWorkers := new(atomic.Uint32)
	numWorkers.Store(50)
	numWorkersStarted := new(atomic.Uint32)
	...
	if totalOps.Load() == 0 || totalOps.Load() != successfulOps.Load() {
		t.Fatalf("total/successful ops zero or mismatch: %d/%d; parallel: %t, num %d", totalOps.Load(), successfulOps.Load(), parallel, num)
	}
	t.Logf("total operations tried: %d, total successful: %d; parallel: %t, num %d", totalOps.Load(), successfulOps.Load(), parallel, num)
}
